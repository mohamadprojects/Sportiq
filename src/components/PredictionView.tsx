...
export const PredictionView: React.FC<PredictionViewProps> = ({
  ...
  const handlePredict = async () => {
    ...
    if (!selectedImage) {
      setError('Please upload a match screenshot or photo first.');
      return;
    }
    ...
    try {
      const res = await fetch('/api/predict', {
        ...
        body: JSON.stringify({
          ...
          mimeType,
          matchContextNotes: matchNotes
        })
      });

      const data = await res.json();
      if (!res.ok) {
        throw new Error(data.message || data.error || 'Prediction calculation failed');
      }
  ...
  // Use latestPrediction or fallback display
  ...
  return (
    <main className="flex-1 p-6 sm:p-8 bg-gradient-to-b from-[#0a0a0a] to-[#050505] overflow-y-auto select-none">
      <div className="max-w-5xl mx-auto flex flex-col h-full">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-8 gap-4">
          <div>
            ...
            <p className="text-gray-400 text-sm">
              Upload live match odds, line-ups, stats, or broadcast scoreboards for instant neural quant analysis.
            </p>
          </div>
          <div className="sm:text-right flex sm:flex-col items-center sm:items-end justify-between bg-white/5 sm:bg-transparent p-3 sm:p-0 rounded-xl border sm:border-0 border-white/10">
            <div className="text-[11px] text-gray-500 uppercase font-bold tracking-widest sm:mb-1">
              Engine Quant Accuracy
            </div>
            <div className="text-2xl sm:text-3xl font-mono font-bold text-blue-500 flex items-center gap-1.5">
        ...
        {/* Error Notice */}
        {error && (
          <div className="mb-6 p-4 rounded-2xl bg-red-500/10 border border-red-500/30 text-red-300 text-sm flex items-start gap-3 animate-shake">
            <AlertTriangle className="w-5 h-5 text-red-400 flex-shrink-0 mt-0.5" />
            <div className="flex-1">
              <div className="font-bold uppercase tracking-wider text-xs text-red-400 mb-0.5">Prediction Guard Notice</div>
              <div>{error}</div>
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 flex-1">
          ...
          <div className="flex flex-col gap-4">
            ...
            <div
              ...
              className={`flex-1 min-h-[260px] border-2 border-dashed rounded-2xl flex flex-col items-center justify-center p-8 transition-all cursor-pointer group relative overflow-hidden ${
                selectedImage
              ...
              {selectedImage ? (
                <div className="w-full h-full flex flex-col items-center justify-center relative">
                  <img
                    src={selectedImage}
                    alt="Uploaded match"
                    className="max-h-56 rounded-xl object-contain shadow-2xl border border-white/10 mb-3"
                  />
                  <div className="text-xs text-blue-400 font-mono bg-black/80 px-3 py-1 rounded-full border border-blue-500/30 flex items-center gap-1.5">
                    <CheckCircle className="w-3.5 h-3.5 text-emerald-400" />
                    <span>Photo loaded • Click to replace</span>
                  </div>
                </div>
              ) : (
                <>
                  ...
                  <button
                    type="button"
                    className="mt-6 px-6 py-2 bg-white/10 rounded-full text-xs font-bold uppercase tracking-widest hover:bg-white/20 text-white transition-colors"
                  >
                    Browse Files
                  </button>
                </>
            ...
            <div>
              <label className="text-[11px] font-mono uppercase text-gray-400 font-bold block mb-1.5">
                Optional Match Notes / League Info
              </label>
              <input
                type="text"
                value={matchNotes}
                onChange={(e) => setMatchNotes(e.target.value)}
                placeholder="e.g., Premier League, Arsenal missing Odegaard due to injury"
                className="w-full bg-[#050505] border border-white/10 rounded-xl px-4 py-3 text-xs text-white placeholder-gray-600 focus:outline-none focus:border-blue-500 font-sans"
              />
            </div>

            {/* START AI PREDICTION BUTTON */}
            <button
              type="button"
              onClick={handlePredict}
              disabled={analyzing || !selectedImage}
              className={`w-full py-4 rounded-xl font-bold flex items-center justify-center gap-3 tracking-widest transition-all uppercase shadow-xl ${
                analyzing
                  ...
                  : 'bg-blue-600 hover:bg-blue-500 text-white shadow-blue-600/30 transform active:scale-[0.99]'
              }`}
            >
              {analyzing ? (
                <>
                  <Sparkles className="w-5 h-5 animate-spin text-blue-200" />
                  <span>NEURAL QUANT ANALYZING...</span>
                </>
              ) : !canPredict ? (
                <>
                  <Lock className="w-4 h-4 text-amber-400" />
                  <span>START AI PREDICTION (VIP LOCKED)</span>
                </>
              ) : (
                <>
            ...
            <p className="text-[10px] text-center text-gray-500">
              * Requires Weekly 5 USDT VIP Verification by Administrator
            </p>

          </div>

          {/* Right: Real-time Analysis Result Box */}
            ...
            <div className="p-4 border-b border-white/10 bg-white/5 flex items-center justify-between">
              ...
              <span className="flex items-center gap-1.5 bg-black px-2.5 py-1 rounded-full border border-white/10">
                ...
                <span className="text-[10px] font-mono text-gray-300">
                  {analyzing ? 'Processing Matrix' : activePred ? 'Calculated' : 'Awaiting Signal'}
                </span>
              </span>
            </div>
              ...
              {analyzing ? (
                <div className="my-auto space-y-6 animate-pulse">
                  <div className="text-center space-y-3">
                    <div className="w-16 h-16 rounded-full bg-blue-600/20 border border-blue-500/40 flex items-center justify-center mx-auto animate-spin">
                ...
                <div className="space-y-6 animate-fadeIn">
                  ...
                  <div className="flex items-start justify-between pb-4 border-b border-white/10 gap-2">
                    ...
                    <div className="text-right flex-shrink-0 bg-blue-950/40 border border-blue-500/30 p-3 rounded-xl">
                      ...
                      <div className={`text-[9px] font-bold uppercase mt-0.5 ${activePred.riskLevel === 'Low' ? 'text-emerald-400' : activePred.riskLevel === 'Medium' ? 'text-amber-400' : 'text-rose-400'}`}>
                        Risk: {activePred.riskLevel}
                      </div>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="h-2.5 bg-white/10 rounded-full overflow-hidden flex shadow-inner">
                      <div style={{ width: `${activePred.moneyline.homeWin}%` }} className="h-full bg-blue-500" title={`Home: ${activePred.moneyline.homeWin}%`} />
                      <div style={{ width: `${activePred.moneyline.draw}%` }} className="h-full bg-slate-500 border-l border-r border-black/30" title={`Draw: ${activePred.moneyline.draw}%`} />
                      <div style={{ width: `${activePred.moneyline.awayWin}%` }} className="h-full bg-teal-400" title={`Away: ${activePred.moneyline.awayWin}%`} />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="p-4 rounded-xl bg-white/5 border border-white/10">
                      ...
                      <div className="flex items-baseline justify-between">
                        <span className={`text-lg font-mono font-bold ${activePred.btts.prediction === 'Yes' ? 'text-emerald-400' : 'text-rose-400'}`}>
                          {activePred.btts.prediction}
                        </span>
                    ...
                    <div className="p-4 rounded-xl bg-white/5 border border-white/10">
                      ...
                      <div className="text-base font-mono font-bold text-blue-400 truncate" title={activePred.favorite}>
                        {activePred.favorite}
                      </div>
                  ...
                  {/* Sharp Value Bet Recommendation */}
                  <div className="p-4 rounded-xl bg-gradient-to-r from-blue-900/40 via-slate-900 to-emerald-950/40 border border-blue-500/30">
                    <div className="text-[10px] font-mono text-emerald-400 font-bold uppercase tracking-widest flex items-center gap-1.5 mb-1">
                      <Zap className="w-3.5 h-3.5 fill-current" />
                      <span>Recommended Value Bet</span>
                    </div>
                  ...
                  <div>
                    <div className="text-[10px] uppercase font-mono text-gray-400 font-bold mb-2 tracking-wider">
                      Key Quant Tactical Insights
                    </div>
                ...
                <div className="my-auto space-y-6">
                  <div>
                    <div className="flex justify-between items-end mb-1 opacity-40">
                      <span className="text-xs font-mono uppercase text-gray-400 font-bold">Moneyline</span>
                  ...
                  <div className="grid grid-cols-2 gap-4 opacity-40">
                    <div className="p-4 rounded-xl bg-white/5 border border-white/10">
                      <div className="text-[10px] text-gray-500 uppercase font-bold mb-1">BTTS (Yes/No)</div>
                      <div className="text-lg font-mono text-gray-400">ANALYZING...</div>
                    </div>
                    <div className="p-4 rounded-xl bg-white/5 border border-white/10">
                      <div className="text-[10px] text-gray-500 uppercase font-bold mb-1">Match Favour</div>
                      <div className="text-lg font-mono text-gray-400">PENDING...</div>
                    </div>
                  </div>
                  <div className="mt-auto">
                    <div className="text-[11px] text-blue-500/70 italic border-t border-white/5 pt-4 text-center">
                      Neural network idling. Ready for visual data input. Upload match photo & click predict to generate real-time probability vectors.
                    </div>
                  </div>
                </div>
              )}
