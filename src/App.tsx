...
export default function App() {
  ...
  const [settings, setSettings] = useState<AdminSettings>({
    ...
    systemPrompt: 'You are SportiQ AI Quant Coach...',
  ...
  const fetchData = async () => {
    try {
      ...
      const predRes = await fetch(`${API}/api/predictions`);
      if (predRes.ok) {
        const predData = await predRes.json();
        if (predData.predictions) {
          setPredictions(predData.predictions);
          if (predData.predictions.length > 0) setLatestPrediction(predData.predictions[0]);
        }
      }
    } catch (err) {
  ...
  const checkExistingSession = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.user.role === 'admin') fetchAdminUsers(data.user.username);
      } else {
  ...
  const handleLoginSuccess = (user: User) => {
    setCurrentUser(user);
    if (user.role === 'admin') fetchAdminUsers(user.username);
  };
  ...
  return (
    <div className="bg-[#050505] text-gray-100 min-h-screen w-full font-sans flex flex-col overflow-x-hidden selection:bg-blue-500 selection:text-white">
      <Navbar currentUser={currentUser} onOpenAuth={() => setAuthModalOpen(true)} onLogout={handleLogout} activeTab={activeTab} setActiveTab={setActiveTab} />
      <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
        <Sidebar currentUser={currentUser} settings={settings} onUserUpdate={(updated) => setCurrentUser(updated)} onOpenAuth={() => setAuthModalOpen(true)} setActiveTab={setActiveTab} />
        {activeTab === 'predict' && <PredictionView currentUser={currentUser} settings={settings} onOpenAuth={() => setAuthModalOpen(true)} onNewPrediction={handleNewPrediction} latestPrediction={latestPrediction} />}
        {activeTab === 'feed' && <FeedView predictions={predictions} onSelectPrediction={handleSelectFeedItem} />}
        {activeTab === 'admin' && <AdminView users={usersList} settings={settings} onRefreshUsers={() => fetchAdminUsers()} onUpdateSettings={(newSet) => setSettings((prev) => ({ ...prev, ...newSet }))} currentUser={currentUser} />}
      </div>
      <footer className="h-8 bg-blue-600 flex items-center justify-between px-6 select-none flex-shrink-0 text-white z-40">
        ...
        <div className="text-[10px] text-blue-100 font-medium">SportIQ Proprietary Engine © 2026</div>
      </footer>
      <AuthModal isOpen={authModalOpen} onClose={() => setAuthModalOpen(false)} onSuccess={handleLoginSuccess} />
    </div>
  );
}
